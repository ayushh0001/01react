...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (error.code === 'ERR_NETWORK') {
        setAlertMessage('CORS error: Backend needs to allow your domain. Contact backend developer.');
      } else {
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/auth/forgetPassword/sendOTP`, {
        email: name.includes('@') ? name : '',
        mobile: !name.includes('@') ? name : ''
      }, {
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      setAlertMessage(response.data.message || 'Password reset OTP sent successfully!');
      setAlertType('success');
    } catch (error) {
      if (error.code === 'ERR_NETWORK') {
        setAlertMessage('CORS error: Backend needs to allow your domain.');
      } else {
        setAlertMessage(error.response?.data?.error || 'Failed to send reset OTP.');
      }
      setAlertType('error');
    }
  };
