...
router.post('/new-order', async (req, res) => {
  ...
  // vendorOrderId = UUID of the row in vendor `orders` table — used by Accept Order button
  ...
  // Broadcast with vendorOrderId so the popup's Accept button patches the right row
...
router.post('/new-order', async (req, res) => {
  ...
  // Always broadcast SSE — use vendor orderId if available, else fall back to website id
  broadcastNewOrder({ id: vendorOrderId || id, orderNumber, customerName, totalAmount, itemCount, items });
  res.json({ success: true });
});
